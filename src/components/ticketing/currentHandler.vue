...
<div  v-if="mentionCheck" style="z-index:999999" class="border border-solid border-black w-[350px] h-[300px] fixed bottom-[10px] right-[2px] bg-white  flex flex-col justify-between">
 ...
 <!-- mention end  -->
   ...
   <div class="flex flex-row w-full h-[90vh]   " ref="main">
   ...
   <div  class="flex flex-col bg-white text-black  w-[900px] h-[500px] pt-10  mt-10 px-10 fixed overflow-y-scroll border-2 border-black border-solid" style="top:50px; left:250px; display:none; transition: 0.50s; z-index: 1233333333333333333;" ref="details" >
      ...
      <template v-for="(action, actionCounter) in reversedActions" :key="actionCounter">
                  ...
                  <div class="flex flex-col bg-white mt-10" >
                        ...
                        <hr class="border border-solid border-black w-2/4"/>  
                        ...
                        <div class="flex flex-row pl-10 ">
                           ...
                           <div class="w-2/4 ml-4">{{ action.serial }}</div>
                        </div>
...
import SocketService from './services/socketService';
   ...
   //       handleInput() {
   //    // Logic to identify mentions and show suggestions
   //    const typedText = this.note.slice(this.note.lastIndexOf('@') + 1);
   //    this.showSuggestions = typedText.length > 0;
      
   //    // Filter user data to generate suggestions
   //    this.suggestions = this.users.filter(user =>
   //      user.empName.toLowerCase().includes(typedText.toLowerCase())
   //    );
   //  },
   //  selectSuggestion(suggestion) {
   //    // Logic to insert the mention into the comment
   //    const mention = `@${suggestion.empName} `;
   //    this.note = this.note.replace(/@\w+\s*$/, mention);
   ...
   //  },
    ...
    const message = "this is the message from the current handler";
  ...
  behavior: 'smooth'
   ...
   handleModalCall(value){
      ...
      if(value == 'Supervisor Approved'){
         ...
         console.log("entered ticket assigned in modal call")
      ...
      this.closeTicket(this.comment, this.ticket, this.rating)
