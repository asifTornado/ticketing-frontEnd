<template>
    ...
    <div class="flex flex-row flex-wrap  justify-end items-end  w-full pb-10">  
     ...
     <div class="flex flex-row justify-end items-end flex-wrap" v-if="ticket.currentHandler && ticket.currentHandler.mailAddress == user.mailAddress || user.userType == 'admin'">
       ...
       <!-- <template v-else-if="lastAction.type == 5 || lastAction.type == 12">
           
           <button  class="bg-green-500 text-white font-bold mr-2 mt-2 p-2 rounded-sm" @click="$emit('call', 'Close Request')">Close Request</button> 
           <button  class="bg-blue-500 text-white font-bold mr-2 mt-2 p-2 rounded-sm" @click="$emit('call', 'Asking For Information')">Ask For Info</button>
           <button  class="bg-fuchsia-500 text-white font-bold mr-2 mt-2 p-2 rounded-sm" @click="$emit('call', 'Reassign')">Reassign</button>
       </template> -->
