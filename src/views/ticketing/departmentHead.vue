    ...
    <table class="w-full text-md text-left text-gray-500 dark:text-gray-400 ">
        ...
        <tbody>
            <tr  class="hover:bg-gray-200 hover:text-black" v-for="(ticket, ticketCounter) in sortedTickets" :key="ticketCounter">
                ...
                <td  class="table-row2 ">
                    <template v-if="ticket.assignedTo && ticket.ticketingHead && user.empName == ticket.ticketingHead.empName">
...
import { useUserStore } from '../../stores/users';
    ...
    var {handlePageChanged, getLocations, getTickets, getTeams, getSelectedItem, selectItem, filter, showDetails, downloadExcel, ticketReset} = useTicketStore()
