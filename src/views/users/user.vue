<template>
<div id="app" class="flex flex-col w-[100vw] h-[92vh] overflow-y-scroll justify-start items-center p-10">
...
export default{
    ...
    methods:{
        ...
        processSupportTickets(){
             ...
             axios.post(vm.globalUrl + "getTicketsForSupport", data).then((result)=>{
                var user = vm.user
                ...
                vm.handled = handled.length
                     ...
                     console.log("these are the handled along with the length")
                     console.log(handled)
                     console.log(vm.handled)
        ...
        processLeaderTickets(){
                ...
                axios.post(vm.globalUrl + "getTicketsForLeader", data).then((result)=>{
                    var user = vm.user;
                    vm.tickets = result.data
        ...
        processNormalTickets(){
                 ...
                 axios.post(vm.globalUrl + "getTicketsForNormal", data).then((result)=>{
                    vm.tickets = result.data
                    vm.raised = vm.tickets.length
                    console.log("these are the normal tickets")
                    console.log(vm.tickets)
                    vm.pending = vm.tickets.filter((ticket)=>ticket.status != "Closed Ticket").length
                    console.log("these are the pending")
        ...
        getAverageResponseTime(tickets){
                ...
                console.log("number of tickts")
                console.log(numberOfTickets)
                var respondedTickets = tickets.filter((ticket)=>ticket.actions.length > 1)
                console.log('responded tickets length')
                console.log(respondedTickets.length)
        ...
        getAverageAcceptanceTime(tickets){
            var numberOfTickets = tickets.length
                ...
                console.log('accepted tickets length')
                console.log(acceptedTickets.length)
        ...
        getAverageResolutionTime(tickets){
            var numberOfTickets = tickets.length
                ...
                console.log('resolved tickets length')
                console.log(resolvedTickets.length)
